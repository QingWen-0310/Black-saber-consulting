...
- Whether an employee gets a promotion depends on its productivity, leadership_for_level, and the gender. Gender makes a difference when considering the employee's leadership for level and 
...
Prefer not to say & 0.026             & 0.010    & 0.011             & 0.000    &  0.000                 &   0.000       \\ \hline
...
From *Table 2*, we see that among those candidates who successfully passed Phase-I, 48% of them are male and 51% of them are female, with the rest 1% having a gender of "prefer not to say". Hence, from the table summary, it seems that the Phase-I hiring is pretty fair across genders as the differences across the proportions are small.
...
knitr::kable(hire2_mod_table, align = c("r", "r"), caption = "Hiring phase-II summary output")
```
...
- *financial_q*, categorical, the working period of the employee
- *role_seniority*, categorical, the employee's job position at the company
- *leadership_for_level*,categorical, quality of demonstrated leadership, taking into account of the role level of the employee
- *productivity*, numerical, work output of the employee, productivies of 50 are considered as satisfactory, and above 50 indicates better than expected
...
### Data Wrangling
In the original current employees data, we were given the information of all *current* employees in Black Saber Company for their whole employment duration. The data were collected from 2013 Q2 to 2020 Q4, and it contained basic information of employees such as *gender*, *team* and *employee_id*. Moreover, for each employee, there were multiple quarterly observations recording his/her *productivity*, *leadership_for_level*, *role_seniority* and  *salary*. 

Before doing any data exploration, we made several changes to the given current employees data in order to better observe each employee's promotion situation. We created a new indicator variable named *get_promotion* (ie. if one get promotion in the following next quarter, then the indicator variable returns *1* for current quarter; otherwise, it returns *0*). In this way, it was necessary for us to delete each employee's the most recent quarter observation because they could not provide any relevant promotion information. The above manipulation made sense because the decision of promoting one or not supposed to rely on one's current quarter's performance. Also, we excluded the employees who filled in "Prefer not to say" for gender since here we were interested to investigate the effect of gender on getting promotion.


```{r}
##wrangle tidy data for modeling(answering question 2 and 3)
...
We filtered and focused on the observations only where employees got promotion and received "Appropriate for level" as feedback of *leadership_for_level*.Then, we further grouped them by *gender* and *role_seniority*, and calculated the average productivity for each group. Visually, from the following boxplot *Figure 3*, we could see that female had a relatively higher mean productivity than males did in almost every role level promotion stage. Here, we could interprete such average productivity as the potential productivity threshold of promotion requirement. That was saying under the circumstance where males and females all received the same rating of *leadership_for_level* , females were often required to have a relatively higher average productivity than males did in almost every role level promotion stage. 
...
Next,through a even more detailed division, we grouped observations by *gender*, *role_seniority* and also *team*. In *figure 4*, it can be seen that in many departments such as Data, Marketing and Operations, almost at every  *role_seniority*, females often processed with a considerably higher productivity,further illustrating our previous discovery. Additionally, we noticed that in design department and director level in many departments, there were only males getting promotions. The above data exploration make us concern about whether the promotion decision is made purely on employees' performance, or *gender* had became one of considerations during promotion decision. Hence, we continued investigating through finding a proper modeling.
...
```{r}
##without gender,leadership_for_level
promotion_model1<-lme4::glmer(get_promotion~productivityC+(1|employee_id), data=tidy_data, family = "binomial")
#summary(promotion_model1)
```

```{r}
##without gender
promotion_model2<-lme4::glmer(get_promotion~leadership_for_level+productivityC+(1|employee_id), data=tidy_data, family = "binomial")
...
```{r}
## with gender
promotion_model3<-lme4::glmer(get_promotion~gender+leadership_for_level+productivityC+(1|employee_id),data = tidy_data,family = "binomial")
#summary(promotion_model3)
#lmtest::lrtest(promotion_model2,promotion_model3)
```

**Table 5**: Likelihood ratio test results for model with/without gender

|   | #Df | Loglik | Df | Chisq | Pr(>Chisq) |
|---|-----|--------|----|-------|------------|
| 1 | 5   | -1405.2|    |       |            |
| 2 | 6   | -1402.5| 1  | 5.205 | 0.02253    |


Since we were considering a binary response variable(*get_promotion*) and also having repeated quarterly measurements of each employees, we chose to use the logisric mixed effect linear model. In detailed, we considered  *productivity* and *leadership role* as our fixed effects since they are measures of employees' value to the firm. Furthermore, we are interested in comparing which one of these two factor will have a larger impact. More importantly, we want to investigate whether *gender*  is one of the consideration within one's promotion decision.
...
$\boldsymbol X$: model matrix for the fixed effects: *productivity*, *leadership role* and *gender* \
...
1.our subjects(employees) are independent 

2.random effects come from a normal distribution There is an assumption that random effects—both intercepts and slopes—are normally distributed.)
...
*Table 6* shows the average *productivity* and *salary* by gender. From the table, we see that females possess the highest average productivity of 50.44, but the lowest average salary among the three genders. On the other hand, males have the second highest average productivity of 48.99 and the highest average salary. 
...
**Table 7**: Likelihood ratio test results for model with/without gender
...
Likelihood ratio test is used here to assess whether we should add the specific variable in our model. It will tell us the goodness of fit of two models based on the ratio of their likelihoods. After building models that include *employee id* as only random intercept and random intercept and slope (interacting with *gender*), the likelihood ratio test tells us to adopt the model with only the random intercept (p = 0.9246). Furthermore, to determine whether *gender* plays a significant role in the determination of salary, we compare our current model with a nested model that have the same covariates as in our current model but without *gender*. The result of the model comparison using the likelihood ratio test is presented in *Table 7*. We see that the p-value appears to be really small (< 2.2e-16), so we expect having *gender* to improve our model by a significant amount. 

By including *gender* in our model, the summary outputs for the final model is presented in *Table 8*, we see that the grand average of the salary is 31900.93. Predictors that are significant are: *leadership role: exceeds expectations*, *leadership role: needs improvement*, and all levels of the *seniority role*, since the 95% confidence intervals for these coefficients do not include 0. Among these important covariates, both levels of the *leadership role* have large negative effects, while all levels of the *seniority role* have large positive effects on salary.
...
There is a slight discrepancy that our likelihood ratio test and 95% confidence interval give us different results. The likelihood ratio test says we should use the one with gender included (as gender may impose large effect on average salary), but 95% C.I. gives us the opposite as it is insignificant. Based on the evidence from previous proportion tables and visualizations, that all give us consistent result saying there is a gender bias problem, we tend to believe that there are still some differences when gender comes in, both in hiring processes and employees' benefits. 
