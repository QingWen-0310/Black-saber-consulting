...
cover_success <- hire_phase1_temp %>% 
...
knitr::kable(hire1_mod_table, align = c("r", "r"), caption = "Coefficient estimates for the logistic model built for hiring phase-I")
# summary(hire1_mod)
```
...
### lrt on two models (w/ and w/o gender)

After building two different models, one has gender as a factor and the other one does not, we conduct a likelihood ratio test to verify our final decision of the model. As the P value is larger than 0.05 significance level, indicates there is not much difference between two models and gender does not impose significant effects on the passing rate in this current phase. In this case, we will continue to use the simpler model, which is the one which only has GPA, extracurricular activities and work experiences as factors.
